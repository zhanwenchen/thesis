...
\section{Introduction to Ultrasound Beamforming}
 ...
 In practice, ultrasound scans are acquired with an array of transducers and each transducer element's pulse transmission is preciously timed by a computer. % Because a single transducer element is only able to capture a single depth-dimension signal, a (horizontal) stack of such transducers can introduce the lateral (space) dimension.
 ...
 In the case of plane wave imaging, each transducer element samples its measured acoustic pressure at each timestep within a small time window. The distance to an object is then calculated as
